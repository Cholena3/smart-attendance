...
export default function NotificationBell() {
  ...
  useEffect(() => {
    fetchNotifications();
    // Poll every 30s for new notifications (student app doesn't have socket.io-client)
            ...
            {notifications.length === 0 ? (
              ...
              notifications.map((n) => (
                <div key={n._id}
                  ...
                  className={`px-4 py-3 border-b last:border-0 cursor-pointer hover:bg-gray-50 transition ${!n.read ? 'bg-green-50' : ''}`}>
                  <div className="flex gap-2 items-start">
                    ...
                    {!n.read && <span className="w-2 h-2 bg-green-500 rounded-full mt-1.5 flex-shrink-0"></span>}
                  </div>
                </div>
              ))
            )}
